...
class Trainer:
    ...
    def train(self):
        for e in range(self.EPOCHS):
            ...
            discriminator_metrics = self.discriminator.Discriminator.train_on_batch(x_batch, y_batch)

            # Generate Noise
            x_latent_space_samples = self.sample_latent_space(self.BATCH)
            y_generated_labels = np.ones([self.BATCH, 1])
            generator_metrics = self.gan.gan_model.train_on_batch(x_latent_space_samples, y_generated_labels)

            # print ('Epoch: '+str(int(e))+', [Discriminator :: Loss: '+str(discriminator_loss)+'], [ Generator :: Loss: '+str(generator_loss)+']')
            print('Epoch: ' + str(int(e)), \
                  '[Discriminator ::' + str(self.discriminator.Discriminator.metrics_names[0]) + ': ' + str(
                      discriminator_metrics[0]) + ', ' + \
                  str(self.discriminator.Discriminator.metrics_names[1]) + ': ' + str(discriminator_metrics[1]) + ']', \
                  ' [ Generator ::' + str(self.gan.gan_model.metrics_names[0]) + ': ' + str(generator_metrics[0]) + ', ' + \
                  str(self.gan.gan_model.metrics_names[1]) + ': ' + str(generator_metrics[1]) + ']')

            if e % self.CHECKPOINT == 0:
                self.plot_checkpoint(e)
                self.Discriminator_loss.append(discriminator_metrics[0])
                self.Discriminator_acc.append(discriminator_metrics[1])
                self.Generator_loss.append(generator_metrics[0])
                self.Generator_acc.append(generator_metrics[1])
        return
    ...
    def plot(self):
        ...
        plt.plot(self.Discriminator_acc)
        plt.plot(self.Generator_acc)
        plt.title('model accuracy')
        plt.ylabel('accuracy')
        plt.xlabel('epoch')
        plt.legend(['Discriminator', 'Generator'], loc='upper left')
        # plt.show()
        plt.savefig("./acc.png")  # 和plt.show()不共存
        plt.close('all')

        # summarize history for loss
        plt.plot(self.Discriminator_loss)
        plt.plot(self.Generator_loss)
